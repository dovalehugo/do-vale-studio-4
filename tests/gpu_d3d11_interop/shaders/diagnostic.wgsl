// Green-frame visual diagnostic shaders (Experiment 2).
// Shared fullscreen triangle; fragment entry points select the diagnostic mode.
...
@fragment
fn fs_constant(_input: VertexOutput) -> @location(0) vec4<f32> {
    // Cyan — proves shader/pipeline path without sampling.
    return vec4<f32>(0.0, 1.0, 1.0, 1.0);
}

@fragment
fn fs_plane0_y_only(input: VertexOutput) -> @location(0) vec4<f32> {
...
@fragment
fn fs_plane1_raw(input: VertexOutput) -> @location(0) vec4<f32> {
    let tc = vec2<f32>(input.tex_coords.x, input.tex_coords.y * VISIBLE_V_SCALE);
...
@fragment
fn fs_full_nv12(input: VertexOutput) -> @location(0) vec4<f32> {
    let tc = vec2<f32>(input.tex_coords.x, input.tex_coords.y * VISIBLE_V_SCALE);
    let y = textureSample(y_plane, tex_sampler, tc).r;
    let uv = textureSample(uv_plane, tex_sampler, tc).rg;
...
// Synthetic control uses 1920x1080 — no 2176 crop.
@fragment
fn fs_synthetic_full(input: VertexOutput) -> @location(0) vec4<f32> {
